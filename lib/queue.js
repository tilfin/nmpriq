...
exports.initialize = (dbUri, param, mylogger, callback) => {
  logger = mylogger;
  ...
  MongoClient.connect(dbEndpoint, otherParams, (err, client_) => {
    if (err) {
      callback(err)
      return
    }
    client = client_
    db = client_.db(dbName)
    callback(null)
  })
}
...
exports.dataIn = function(target, items, callback){
  db.collection(target + suffix, function(err, coll) {
      if (err) {
        callback(err);
        return;
      }
