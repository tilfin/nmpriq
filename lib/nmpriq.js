...
class HttpServer {
  ...
  async start(param) {
    this._datamodel = new Datamodel(this._logger)
    await this._datamodel.initialize(this._dbUri, param || null, this._logger)

    this._svcore = servercore(this._datamodel, this._logger)
  ...
  async restart() {
    this._logger.info('server restarting...')
    await this.stop()
    await this.start()
  }
}
